...
import ffmpeg
import numpy as np
from generator_lstm_predict import MyGenerator
...
def get_autoencoder(inputs):                             
    ...
    #encoder
    lstm1 = LSTM(16, return_sequences=False)(inputs)
    repeat = RepeatVector(19)(lstm1)
    #decoder
    lstm2 = LSTM(int(inputs.shape[-1]), return_sequences=False)(repeat)
    return lstm2
...
data_count = len(data)
separator = int(data_count*0.008)
...
for i in range(8):
    ...
    lstm_inputs = Input(shape=(19, lstm_cells))
    ...
    autoencoder.save("model_lstm_predict" + str(i) + ".h5")
