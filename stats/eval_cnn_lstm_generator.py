...
data_generator = MyGenerator(data_set, batch_size=batch_size, dim=input_dims, shuffle=shuffle)


print(np.shape(X))
X = np.array(X)

y = my_model.predict_generator(data_generator)
y = y[:,19]
y = y.reshape(-1, 16)
